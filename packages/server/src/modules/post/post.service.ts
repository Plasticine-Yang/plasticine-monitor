...
export class PostService {
  ...
  async create(createPostDto: CreatePostDto) {
    ...
    if (post) {
      throw new BusinessHttpException(API_CODES.ENTITY_DUPLICATED, `标题为: '${title}' 的文章已存在，请勿重复创建`)
    }
  ...
  findOne(id: number) {
    try {
      ...
      throw new BusinessHttpException(API_CODES.ENTITY_NOT_EXIST, `文章不存在 -- ${error}`, {
